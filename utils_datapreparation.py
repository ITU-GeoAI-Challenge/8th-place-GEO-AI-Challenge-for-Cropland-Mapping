...
def compute_indices(X_3d):
    ...
    EVI = np.divide(X_3d[:,:,6] - X_3d[:,:,2],  (X_3d[:,:,6] + (6 * X_3d[:,:,2]) - (7.5 * X_3d[:,:,0]) + 1 + epsilon)) * 2.5 # enhanced vegetation index
    ...
    NDPI = np.divide(X_3d[:,:,9] - X_3d[:,:,1], (X_3d[:,:,9] + X_3d[:,:,1]) + epsilon) # normalized difference pond index
    WRI =  np.divide(X_3d[:,:,1] + X_3d[:,:,2], (X_3d[:,:,6] + X_3d[:,:,9]) + epsilon) # water ration index
    PSRI = np.divide(X_3d[:,:,2] - X_3d[:,:,0], X_3d[:,:,6]+  epsilon) # plant senescence reflectance index
    DVI  = X_3d[:,:,6] - X_3d[:,:,2] # difference vegetation index
    RVI = np.divide(X_3d[:,:,6] , X_3d[:,:,2]+ epsilon) # ratio vegetation index
